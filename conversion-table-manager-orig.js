    ...
    class ConversionTable {
        ...
        register(name, table, force = false) {
            try {
                ...
                [result, data] = instance.normalizeTable(table);
                if (result) {
                    return [result, null]
                }
         ...
         * - Identifies the base unit key and ensures there is only one base unit.
         *
         * If multiple base units are found, it returns an error message with details.
         *
         * @param {Object} table - The conversion table to normalize. Each unit can have:
         *    - **base**: Marks the unit as the base unit (optional, only one unit should have `base: true`).
         *    - **scale**: Defines the scale factor relative to the base unit (default is 1).
         *    - **bias**: Offset added after scaling, useful in conversions like temperature (default is 0).
         *    - **alias**: Maps this unit to another unit in the table (e.g., 'in' might alias 'inch').
         *    - **minor**: Optional minor unit associated with the main unit (e.g., 'pica' has minor unit 'points').
         *
         * @returns {[string|null, Object|null]} - Returns a tuple:
         *    - On success: `[null, { table: normalizedTable, base: baseKey }]` where:
         *        - `normalizedTable`: The normalized conversion table.
         *        - `baseKey`: The key for the base unit.
         *    - On error (e.g., if multiple base units are found): `[errorString, null]`.
         */
        normalizeTable(table) {
            ...
            for (const key in table) {
                ...
                if (value.base === true) {
                    if (baseKey !== '') {
                        return [`Duplicate Base Key in table:${table}, ${key} collides with ${baseKey}`, null]
                    }
                    baseKey = key
                    normalizedTable[key] = {
                        ...value,
            ...
            return [ null, { table: normalizedTable, base: baseKey } ];
        }
        ...
        regex(name) {
            if (!this.tables[name]) {
                return this.notRegisteredError(name);
            }
        ...
        parse(input, tableName) {
            const myTable = this.tables[tableName]
            ...
            if (match && match.groups) {
                ...
                // get the table
                const myTableInstance = this.tables[tableName].instance;

                // Retrieve the relevant table entry (handling alias resolution)
                let tableEntry = myTableInstance.table[majorUnit];
                ...
                if (tableEntry && tableEntry.alias) {
                    ...
                    tableEntry = myTableInstance.table[parentUnit];
                }
                ...
                if (minorValue && tableEntry && tableEntry.minor) {
                    ...
                    subUnit = {
                        ...
                        scale: myTableInstance.table[minorUnit]?.scale || 1,
                        bias: myTableInstance.table[minorUnit]?.bias || 0,
                    };
                }

                return [null, {
                    main: {
                        unit: parentUnit || myTableInstance.base,  // Use the resolved parent unit (alias resolved)
